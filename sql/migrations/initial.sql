...
-- Created: 2025-09-22
-- Merged with newsletter extensions on 2025-09-26
...
END $$;
DO $$ BEGIN CREATE TYPE lead_type AS ENUM ('client', 'candidate', 'partner', 'newsletter');
EXCEPTION
WHEN duplicate_object THEN NULL;
END $$;
...
WHEN duplicate_object THEN NULL;
END $$;
DO $$ BEGIN CREATE TYPE direction AS ENUM ('inbound', 'outbound', 'system');
EXCEPTION
WHEN duplicate_object THEN NULL;
END $$;
...
WHEN duplicate_object THEN NULL;
END $$;
DO $$ BEGIN CREATE TYPE doc_provider AS ENUM ('chaindoc', 'docusign', 'manual', 'other');
EXCEPTION
WHEN duplicate_object THEN NULL;
END $$;
...
WHEN duplicate_object THEN NULL;
END $$;
DO $$ BEGIN CREATE TYPE subscriber_status AS ENUM ('subscribed', 'unsubscribed', 'pending_opt_in');
-- Added 'pending_opt_in'
EXCEPTION
WHEN duplicate_object THEN NULL;
END $$;
DO $$ BEGIN CREATE TYPE availability AS ENUM (
    'available_now',
    'part_time',
    'notice_period',
    'unavailable'
...
WHEN duplicate_object THEN NULL;
END $$;
DO $$ BEGIN CREATE TYPE classification_label AS ENUM (
    'spam',
    'ham',
    'client',
    'partner',
...
WHEN duplicate_object THEN NULL;
END $$;
DO $$ BEGIN CREATE TYPE rate_period AS ENUM ('hourly', 'daily', 'monthly');
EXCEPTION
WHEN duplicate_object THEN NULL;
END $$;
-- ========================================================
...
CREATE TRIGGER set_timestamp BEFORE
UPDATE ON companies FOR EACH ROW EXECUTE FUNCTION trigger_set_timestamp();
CREATE UNIQUE INDEX IF NOT EXISTS companies_website_unique_not_null ON companies (website)
...
CREATE TRIGGER set_timestamp BEFORE
UPDATE ON contacts FOR EACH ROW EXECUTE FUNCTION trigger_set_timestamp();
...
CREATE TABLE IF NOT EXISTS contact_emails (
    ...
    email TEXT NOT NULL,
    label TEXT,
    -- e.g., work, personal, billing
    is_primary BOOLEAN NOT NULL DEFAULT FALSE,
    created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
    updated_at TIMESTAMPTZ NOT NULL DEFAULT now()
);
CREATE UNIQUE INDEX IF NOT EXISTS ux_contact_emails_primary_per_contact ON contact_emails (contact_id)
WHERE is_primary = TRUE;
CREATE UNIQUE INDEX IF NOT EXISTS ux_contact_emails_contact_email_ci ON contact_emails (contact_id, lower(email));
DROP TRIGGER IF EXISTS set_timestamp ON contact_emails;
CREATE TRIGGER set_timestamp BEFORE
UPDATE ON contact_emails FOR EACH ROW EXECUTE FUNCTION trigger_set_timestamp();
CREATE TABLE IF NOT EXISTS contact_phones (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    contact_id UUID NOT NULL REFERENCES contacts(id) ON DELETE CASCADE,
    phone TEXT NOT NULL,
    label TEXT,
    -- e.g., mobile, office, telegram
    is_primary BOOLEAN NOT NULL DEFAULT FALSE,
    created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
    updated_at TIMESTAMPTZ NOT NULL DEFAULT now()
);
CREATE UNIQUE INDEX IF NOT EXISTS ux_contact_phones_primary_per_contact ON contact_phones (contact_id)
WHERE is_primary = TRUE;
CREATE UNIQUE INDEX IF NOT EXISTS ux_contact_phones_contact_phone ON contact_phones (contact_id, phone);
DROP TRIGGER IF EXISTS set_timestamp ON contact_phones;
CREATE TRIGGER set_timestamp BEFORE
UPDATE ON contact_phones FOR EACH ROW EXECUTE FUNCTION trigger_set_timestamp();
...
CREATE TABLE IF NOT EXISTS leads (
    ...
    SET NULL,
        source_id INTEGER REFERENCES sources(id) ON DELETE
    SET NULL,
        score INTEGER,
        created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
        updated_at TIMESTAMPTZ NOT NULL DEFAULT now(),
...
CREATE TRIGGER set_timestamp BEFORE
UPDATE ON leads FOR EACH ROW EXECUTE FUNCTION trigger_set_timestamp();
-- ========================================================
...
CREATE TABLE IF NOT EXISTS submissions (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    form_id INTEGER REFERENCES forms(id) ON DELETE
    SET NULL,
        source_id INTEGER REFERENCES sources(id) ON DELETE
    SET NULL,
        resource TEXT,
        -- which site/app (e.g., chain.do, main site)
        ...
        -- idempotency key from webhook/email
...
CREATE TABLE IF NOT EXISTS messages (
    ...
    sender_contact_id UUID REFERENCES contacts(id) ON DELETE
    SET NULL,
        body TEXT,
        body_html TEXT,
...
CREATE TRIGGER set_timestamp BEFORE
UPDATE ON documents FOR EACH ROW EXECUTE FUNCTION trigger_set_timestamp();
...
CREATE TRIGGER set_timestamp BEFORE
UPDATE ON bookings FOR EACH ROW EXECUTE FUNCTION trigger_set_timestamp();
-- Newsletter subscribers (depends on contacts, sources) - ENHANCED
CREATE TABLE IF NOT EXISTS newsletter_subscribers (
    ...
    SET NULL,
        ...
        updated_at TIMESTAMPTZ NOT NULL DEFAULT now(),
        -- Enhancements for DOI, digests, etc.
...
CREATE TRIGGER set_timestamp BEFORE
UPDATE ON newsletter_subscribers FOR EACH ROW EXECUTE FUNCTION trigger_set_timestamp();
CREATE UNIQUE INDEX IF NOT EXISTS ux_newsletter_subscribers_dedupe ON newsletter_subscribers(dedupe_key)
...
CREATE TRIGGER set_timestamp BEFORE
UPDATE ON candidate_profiles FOR EACH ROW EXECUTE FUNCTION trigger_set_timestamp();
CREATE TABLE IF NOT EXISTS partner_profiles (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    company_id UUID UNIQUE REFERENCES companies(id) ON DELETE CASCADE,
    areas JSONB,
    -- e.g., {'fintech': true, 'ai': true}
    tech_stack JSONB,
    -- e.g., ['react','node','solidity']
    available_specialists JSONB,
    -- list or map
    avg_rates JSONB,
    -- e.g., {'senior': 60, 'middle': 40}
    notes TEXT,
    created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
    updated_at TIMESTAMPTZ NOT NULL DEFAULT now()
);
...
CREATE TRIGGER set_timestamp BEFORE
UPDATE ON partner_profiles FOR EACH ROW EXECUTE FUNCTION trigger_set_timestamp();
...
CREATE TRIGGER set_timestamp BEFORE
UPDATE ON tasks FOR EACH ROW EXECUTE FUNCTION trigger_set_timestamp();
...
CREATE TRIGGER set_timestamp BEFORE
UPDATE ON webhooks FOR EACH ROW EXECUTE FUNCTION trigger_set_timestamp();
...
CREATE TRIGGER set_timestamp BEFORE
UPDATE ON pools FOR EACH ROW EXECUTE FUNCTION trigger_set_timestamp();
...
CREATE TABLE IF NOT EXISTS outreach_campaigns (
    ...
    name TEXT NOT NULL,
    type TEXT CHECK (
        type IN ('partner', 'fake_client', 'hr', 'newsletter')
    ) NOT NULL,
    status TEXT CHECK (
...
CREATE TRIGGER set_timestamp BEFORE
UPDATE ON outreach_campaigns FOR EACH ROW EXECUTE FUNCTION trigger_set_timestamp();
...
CREATE TRIGGER set_timestamp BEFORE
UPDATE ON templates FOR EACH ROW EXECUTE FUNCTION trigger_set_timestamp();
...
CREATE TRIGGER set_timestamp BEFORE
UPDATE ON campaign_steps FOR EACH ROW EXECUTE FUNCTION trigger_set_timestamp();
-- Pipeline event log (independent) - ENHANCED
CREATE TABLE IF NOT EXISTS pipeline_events (
    id BIGSERIAL PRIMARY KEY,
    entity_type TEXT CHECK (
        entity_type IN ('lead', 'candidate', 'partner', 'subscriber')
    ) NOT NULL,
    -- Added 'subscriber'
...
VALUES ('chain_do', 'Chain.do site'),
    ('main_site', 'Main company site'),
    ('email_inbox', 'Email inbox') ON CONFLICT (code) DO NOTHING;
INSERT INTO forms (code, name, type)
VALUES (
        'dev_request',
        'Development / Consulting Request',
        'development_request'
    ),
    (
        'vacancy',
...
VALUES ('followup_1_sent'),
    ('followup_2_sent') ON CONFLICT (name) DO NOTHING;
-- Base templates
INSERT INTO templates (name, channel, subject_template, body_template)
VALUES (
        'Partner outreach: initial',
        'email',
...
{{body}}'
    ...
    (
        'Partner outreach: followup 1',
        'email',
        'Following up on partnership',
        'Hi {{name}},
...
INSERT INTO templates (name, channel, subject_template, body_template)
VALUES (
        'dev_request.initial.en',
        'email',
        'Thanks for reaching out — 15‑min intro call?',
        'Hi {{name}},
...
— {{from_name}}'
    ...
    (
        'dev_request.initial.ru',
        'email',
        'Спасибо за обращение — назначим 15‑мин созвон?',
        'Привет, {{name}}!
...
— {{from_name}}'
    ...
    (
        'dev_request.followup1.en',
        'email',
        'Quick reminder to book a 15‑min intro',
        'Hi {{name}},
...
— {{from_name}}'
    ...
    (
        'dev_request.followup2.en',
        'email',
        'Still up for a quick intro?',
        'Hi {{name}},
...
Jeśli to nie Ty, zignoruj tę wiadomość.'
    ...
    (
        'newsletter.welcome.ru',
        'email',
        'Добро пожаловать в наш дайджест',
        'Привет{{full_name_suffix}}!
...
Aby się wypisać kliknij {{unsubscribe_url}}.'
    ...
    (
        'newsletter.confirmed.ru',
        'email',
        'Подтверждение подписки',
        'Подписка подтверждена{{full_name_suffix}}!
...
Управлять подпиской: {{unsubscribe_url}}'
    ...
    (
        'newsletter.digest.en',
        'email',
        '[Digest] {{generated_date}}',
        '{{greeting_line}}
...
Manage your subscription: {{unsubscribe_url}}'
    ...
    (
        'newsletter.digest.pl',
        'email',
        '[Newsletter] {{generated_date}}',
        '{{greeting_line}}
